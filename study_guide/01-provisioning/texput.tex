...
Supposing that you have decided to use Xubuntu in a USB disk,
...
\item Insert now the (at least) 8GB USB drive where Xubuntu will be installed.
...
\item Choose \texttt{Erase disk and install Xubuntu}. Ignore the
  Advanced features. Then, wait for a couple of minutes :-/
...
\item At this point of the installation you should consider (depending
  ...
  \href{https://en.wikipedia.org/wiki/Page_(computer_memory)}{memory
    page} at all. Said that, if you decide to create a specific swap partition,
  click on ``advanced partitioning tool'' and do the modifications you
  want.

\item Very important: check that the boot loader
  (\href{https://www.gnu.org/software/grub/}{GRUB}) will be installed
  in \texttt{/dev/sdc1}.

\item Click on ``Install Now''. You'll read something similar to:
...
\item Configure you personal account (user and password), hostname and
  logging process.
