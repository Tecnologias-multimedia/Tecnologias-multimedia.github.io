...
(bits per second) that it can provide. This bit-rate depend on the
maximum
...
level is time-variying and quite unpredictable.

In this milestone we are going to measure the impact of the
transmission bit-rate on the
\href{https://en.wikipedia.org/wiki/Quality_of_experience}{QoE}. Similarly
to the procedure used for measuring the impact of the jitter, we are
going to use
\href{https://man7.org/linux/man-pages/man8/tc.8.html}{\testtt{tc}} to
control the amount of data that a instance of InterCom will be allowed
to send in a local environment, with the aim of simulating a real
one. Notice that this limitation will also affect to the loss of
