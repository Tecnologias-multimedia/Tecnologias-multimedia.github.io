...
that $\psi=\tilde\phi$, and this is also true for all orthogonal
DWTs. Another important characteristic of orthogonal DWTs is that the
filters cannot be
...
Biorthogonal\footnote{All transforms express a change of basis. When
  ...
  transpose of the analysis transform. However, the synthesis filters
  are orthogonal to their corresponding \emph{dual} analysis filters,
  and in this case the transform is said
  biorthgonal.~\cite{vetterli2014foundations}} filters can be
\href{http://wavelets.pybytes.com/}{easely recognized} because they
are always symmetric and because the analysis filters are different to
the synthesis filters ($\phi\ne\tilde{\phi}$ and
$\psi\ne\tilde{\psi}$).
