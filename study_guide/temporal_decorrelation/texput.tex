...
example, a dead-zone quantizer and most of the coefficients are close
to zero, the quantizer will generate a high number of zero
...
and the transform is in fact ``descomposing'' $x$ into two subbands
$w_0$ and $w_1$ (see the Figure~\ref{fig:PRFB}, and this
\href{}{notebook}). On the other hand, the synthesis transform matrix
$K^{-1}$ denotes the taps of a synthesis FB, and the complete
system\footnote{The concatenation of the analysis and the synthesis
transform.} is called a (2-channels)
\href{https://en.wikipedia.org/wiki/Filter_bank#Perfect_reconstruction_filter_banks}{Perfect
  Reconstruction Filter Bank (PRFB)}. Notice that $x$ can be recovered perfectly from $w_0$ and $w_1$ be
...
MST coefficients that show some degree of temporal correlation, not a
simple pair of samples (a frame).} Let's also suppose (as happens in
the MST) that $K_0$ is a low-pass filter and $K_1$ is a high-pass filter.

Let's suppose, for now, that $K$ has only two
filters (with taps) $K_0$ and $K_1$ (again, see the previous
milestone), and that we apply them to the input signal
$x$, and
that the
...
\href{https://en.wikipedia.org/wiki/Filter_bank#Perfect_reconstruction_filter_banks}{are
  the inverse of each other}. Under these assumptions, the
concatenation of the analysis and synthesis transforms, and the
decomposition performed, is equivalent to the use of a
\href{https://en.wikipedia.org/wiki/Filter_bank#Perfect_reconstruction_filter_banks}{Perfect
  Reconstruction Filter Bank (PRFB)}.
