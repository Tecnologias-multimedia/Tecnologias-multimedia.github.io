...
The matrix $K$ proposed corresponds to the
\href{https://en.wikipedia.org/wiki/Joint_encoding#M/S_stereo_coding}{Mid/Side
  (M/S) stereo coding}~\cite{bosi2003intro}, which is similar to the
$2\times 2$ KLT
...
(set)~\cite{strang4linear}. The basis as also a direct interpretation
...
each subband ($w_0$ and $w_1$) by $2$ after computing the analysis
transform.
...
same quantization step ($\Delta_0=\Delta_1$).
\begin{comment}
Unfortunately, this
...
\item Override the methods pack() and unpack(). In pack() perform the
  ...
  synthesiis transform. These procedures should be applied to all the
