...
(\verb|buffer.py|). Similarly to the procedure used for measuring the impact
of the latency and the jitter, we will use
...
will discard those chunks that can not be buffered in the
retransmission nodes (routers and switches)\footnote{Notice that, in
this case, we would be at least contributing, if not causing, the link
congestion.}.
%}}}
...
distortion~\cite{taubman2002jpeg2000}.
