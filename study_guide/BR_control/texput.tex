...
\href{https://en.wikipedia.org/wiki/Bit_rate}{transmission
  bit-rate}\footnote{Measured in bits per second or a $10$-multiple of
  this transmission
...
the maximum capacity (a characteristic closely related with the
available
...
also affect to the loss of chunks because if the transmission bit-rate
is smaller than the audio bit-rate, sooner of later the link will
discard those chunks that can not be buffered in the retransmission
...
action that also will reduce the bandwidth comsumption in
InterCom. The \verb|pack()| and the \verb|unpack()| methods can
compress and decompress, respectively, the chunks that are
processed. To compress and decompress, we will use a free
...
\href{https://en.wikipedia.org/wiki/Lempel%E2%80%93Ziv%E2%80%93Storer%E2%80%93Szymanski}{LZSS}
  ...
  this
  \href{https://vicente-gonzalez-ruiz.github.io/Huffman_coding/}{notebook}). The DEFLATE algorithm is implemented in the Python's standard library
...
\item
  ...
  Fig.~\ref{fig:reordering}). Notice that with this shuffling, the
  samples are not interleaved and the
...
\href{https://en.wikipedia.org/wiki/Lossy_compression}{lossy
  compression} systems, quantization is the only source of
...
context~\cite{vetterli2014foundations}, and requiring much higher
...
happens with electronic noise), the SNR of the input signal has its
lowest value precisely for those values close to 0. Therefore, the
quantizer basically is going to change the electronic noise by
...
\href{https://github.com/vicente-gonzalez-ruiz/scalar_quantization}{comparative
  between digital scalar quantizers}).

Notice, however, that the previous reasoning does not considers what a
human being would, from a purely subjective point of view,
prefer\footnote{For example, in quiet scenarios, low amplitudes are
  usually important for us ...}.
...
basically on the so called quantization step (size), typically denoted
...
should use a payload as large as possible (ideally, infinite bytes),
in order to make $t_t$ a significative amount of time, ease to
measure. Notice also that we should use the smaller RTT value of a
...
If the devices cannot be \texttt{ping}-ed, and supposing that
