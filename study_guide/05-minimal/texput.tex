...
\verb|pack(chunk)| and \verb|unpack(packet)| operations is
smaller than the chunk-time. The first premise (variable chunk sizes)
complicates slightly the implementation because we would work with
chunks of constantly changing lenghts (that as you can see, in most of
the iterations are 0). This also complicates the control of the latency
...
\item Read the source code of
  ...
  the timer-based algorithm. This
  \href{https://github.com/vicente-gonzalez-ruiz/YAPT/blob/master/03-IO/networking/sockets.ipynb}{guide}~\cite{YAPT}
...
\item Evaluate the performance (quality of the sound) in different configurations:
  ...
  \item When \verb|minimal.py| runs in different computers. In this
