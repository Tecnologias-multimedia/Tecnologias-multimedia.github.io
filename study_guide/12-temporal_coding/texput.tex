...
% An intro to PRFBs
Let's suppose now that the filters (represented by the taps of)
${\mathbf K}_0$ and ${\mathbf K}_1$ are applied to the input signal
${\mathbf x}$ (now a sequence of $N$ samples) using a
...
same bandwidth~\cite{sayood2017introduction}. In this situation, in
which ${\mathbf K}_1$ and ${\mathbf K}_2$ are mirror filters, we say
that they form a
...
the signal $x$, decomposing it in
...
\item Lifting is based on the concept of dyadic
  ...
  \begin{equation}
    ...
    \label{eq:1dwt}
  \end{equation}
  where the $s$-th subband
  ${\mathbf z}^s=\{{\mathbf z}_i^s~|~0\le i\le 2^{n-s}\}$, being
  ...
  In general, for a $s$-levels DWT, we get
    \begin{equation}
    \begin{array}{rcl}
      {\mathbf l}^s_i & = & {\mathbf l}^{s-1}_{2i} + {\mathbf l}^{s-1}_{2i+1} \\
      {\mathbf h}^s_i & = & {\mathbf l}^{s-1}_{2i+1} - {\mathbf l}^{s-1}_{2i}.
    \end{array}
    \label{eq:2dwt}
  \end{equation}

  The $s$-levels DWT splits the signal spectrum in $s+1$ subbands. If
  $s=n$ (where $N=2^n$), we have the spectrum partition
  \begin{equation*}
  ...
  \end{equation*}
  where\footnote{The coefficient ${\mathbf l}^s_0$ is called the DC
...
\item DWT performs a number of lifting steps, each one with
  ...
  \item A \textbf{predict step}, that computes the ${\mathbf h}$
    ...
    the even samples (usually, the values used to predict) and the odd
    samples (usually, the values predicted). For the MST filters, we
    have that
    \begin{equation}
      {\mathbf h}^s_i = {\mathbf l}^{s-1}_{2i+1} - {\mathbf l}^{s-1}_{2i}.
    \end{equation}
  ...
  \item An \textbf{update step}, which computes the $l$ subbands
    ...
    \begin{equation}
      {\mathbf l}^s_i = 2{\mathbf l}^{s-1}_{2i} + {\mathbf h}^s_i.
    \end{equation}
  \end{enumerate}
  ...
  \begin{equation}
    \begin{array}{rcl}
      {\mathbf l}^{s-1}_{2i} & = & \frac{1}{2}({\mathbf l}^s_i - {\mathbf h}^s_i)\\
      {\mathbf l}^{s-1}_{2i+1} & = & {\mathbf l}^{s-1}_{2i} + {\mathbf h}^s_i.
    \end{array}
  \end{equation}
...
high-pass filter (${\mathbf K}_1$) is known as the wavelet function
and it is usually denoted by $\tilde\psi$, the response of the
synthesis low-pass filter (${\mathbf K}^{-1}_0$) is denoted by $\phi$
and the synthesis high-pass filter (${\mathbf K}^{-1}_1$) is
represented by $\psi$.
...
\begin{equation}
  {\mathbf h}^s_i = {\mathbf l}^{s-1}_{2i+1} - \frac{1}{2}({\mathbf l}^{s-1}_{2i} + {\mathbf l}^{s-1}_{2i+2})
\end{equation}
and the update step as
\begin{equation}
  {\mathbf l}^s_i = {\mathbf l}^{s-1}_{2i} + \frac{1}{4}({\mathbf h}^s_{i-1} + {\mathbf h}^s_i),
\end{equation}
...
\begin{equation}
  \begin{array}{rcl}
    {\mathbf l}^{s-1}_{2i} & = & {\mathbf l}^s_i - \frac{1}{4}({\mathbf h}^s_{i-1} + {\mathbf h}^s_i)\\
    {\mathbf l}^{s-1}_{2i+1} & = & {\mathbf h}^s_i + \frac{1}{2}({\mathbf l}^{s-1}_{2i} + {\mathbf l}^{s-1}_{2i+2}).
  \end{array}
\end{equation}
