...
\subsubsection{Analysis transform}
%{{{ 
...
\subsubsection{Synthesis transform}
%{{{ 
...
\begin{comment}
  ...
  is much faster than the described in the previous paragraph, but it
...
(remember that we are working with orthogonal transforms and
therefore, the contribution of the subbands are independent). Thus, if
...
\href{https://en.wikipedia.org/wiki/Lp_space}{L$^2$
  ...
  \href{https://en.wikipedia.org/wiki/Vector_space}{spaces}.} (or
...
the same gain ($1/\sqrt{2}$). This result tell us that both subbands
...
As a final remark, we could also consider that any alternative other than $\Delta_0=\Delta_1$ will affect to the spatial perception of the audio. The notebook \href{https://github.com/Tecnologias-multimedia/Tecnologias-multimedia.github.io/blob/master/study_guide/transform_coding/stereo_transforms_RD.ipynb}{A
...
\href{https://en.wikipedia.org/wiki/Psychoacoustics}{humman perception
  of the sound} depends on the frequency: (as it can be
...
frequency of the sound is low\footnote{And also, we are more sensitive
  to low frequencies that to high ones.}. This fact is related with
...
represented by $\psi$, that are the dual scaling and wavelet functions.
...
they also satisfy that $\psi\bot\tilde\phi$ and
$\phi\bot\tilde\psi$.
...
decomposition should be inversely\footnote{Remember that if the QSS is
  ...
  subbands with a higher amplification in the inverse transform should
  be quantized less.} proportional the gain of the synthesis
filters. The gain of a filter correspond to the L$^2$ norm of its
...
that, if we don't know the taps, we can use the algorithm described in
the Section~\ref{sec:quantization_subbands_spatial} to find the gain
of the subbands.

Finally, notice that if the transform in orthonormal, by definition,
the gain of the analysis and synthesis filters is always 1, and also
this is the gain of the subbands. In the notebook
\href{https://github.com/Tecnologias-multimedia/Tecnologias-multimedia.github.io/blob/master/study_guide/transform_coding/DWT_RD.ipynb}{A
  RD-comparison of DWTs} there is more information about the QSS that
...
\begin{figure}
  ...
  \caption{On the top-left, three consecutive chunks of a real mono
    ...
    \href{https://github.com/Tecnologias-multimedia/intercom/blob/master/docs/quantization_DWT.ipynb}{Quantization
      in the DWT domain}.}
  \label{fig:3_chunks}
\end{figure}
...
\href{https://github.com/Tecnologias-multimedia/Tecnologias-multimedia.github.io/blob/master/study_guide/transform_coding/overlapped_DWT_I.ipynb}{Overlapped
  DWT}, and the result can be seen in the Fig.~\ref{fig:3_chunks}.
...
all the RD curves together (in the same graph), to compare easely,
...
\item Does the computation of the DWT using chunk overlapping increase
  the latency of the whole system? And the jitter? In which amount?

\item Which other transform(s) are used in audio encoding systems
  (such as MP3) for exploiting the temporal redundancy? Enumerate the
  systems and the used transform(s).
...
You can also visualize the DWT domain running the commands that are defined in the notebook \href{https://github.com/Tecnologias-multimedia/InterCom/blob/master/docs/2-hours_seminar.ipynb}{InterCom: a Real-Time Digital Audio Full-Duplex Transmitter/Receiver}.
