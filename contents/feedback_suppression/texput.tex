...
the sound that comes out of our PC's speaker some time later reaches
...
\item $s$ the (analog) signal played by our
  (loud)speaker and that reaches our mic,
\item $n$ the signal emited by
  the near-end person (that's me) that reaches our mic\footnote{I.e.,
  the same signal that would be captured by our mic if I were using a
  headset.}, and
\item $m$ the (mixed) signal recorded by our microphone,
\end{enumerate}
we have that
\begin{equation}
  m(t) = n(t) + s(t),
...
emphasize that $\hat{\mathbf s}$ is a registered\footnote{That matches
  in time. That are synchronized.} prediction for $s$
reaching our microphone.
