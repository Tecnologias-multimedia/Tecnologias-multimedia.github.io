...
frequencies). In general, the expression of a FIR filtering in the
...
\begin{equation}
  ...
  \label{eq:FIR_filter}
\end{equation}
where $N$ is the number of coefficients of the filter ${\mathbf
  h}$. When the (coefficients of the) filter are updated for each
input sample, Eq.~\ref{eq:FIR_filter} can be rewritten as
\begin{equation}
