...
Notice that we have used the notation $\tilde{\cdot}$ to highlight
...
\section{Adaptive filtering in the time-domain}
...
\item In Equations \eqref{eq:simplest} and
  ...
  samples-time) the time that $s$ need to travel from the speaker to
...
Eq.~\ref{eq:eq:minimal_filter} represents the minimal expression for a
\href{https://en.wikipedia.org/wiki/Finite_impulse_response}{FIR
  filter} (whose spectral shape is completely flat because it
attenuates equally all the frequencies). In general, the expression of
a FIR filter operation in the time-domain (applied to a signal
previously delayed $d$ samples) is
\begin{equation}
