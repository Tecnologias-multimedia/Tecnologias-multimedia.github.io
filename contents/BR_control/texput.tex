...
contrary, the throughput is time-variating and quite unpredictable,
because it depends on the congestion level that, in turn, depends
on the behavior of the network users.

In this milestone, we will measure the impact of the link
throughput on the
...
to the procedure used to measure the impact of latency and
jitter, we will use
...
also affect the loss of chunks because if the link capacity is
smaller than the audio bit rate (throughput), sooner or later the
...
%{{{ 
With the bitrate value $b$ calculated from the previous section, simulate
