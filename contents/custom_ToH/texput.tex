...
similar question arises when our audio equipments are not the
best. For this reason, this milestone proposes the calculation of your
own ToH curve, information that must be transmitted to your
interlocutor so that he can use the quantization steps adapted to your
perception.

\section{A procedure to determine your ToH curve}

An optimal-user-specific ToH
should take into consideration your \textbf{noticeable}
...
\item %Let
  ...
  iteration the rest of subbands are set to zero), and while the noise
...
It's a good idea to have also the option of determining the ToH of a given
subband, without reconsidering the rest of them.

\section{Deliverables}

Implement the previous procedures in a Python module named
...
\verb|custom_ToH.txt|, when available. Use the frequencies
corresponding to those defined in \verb|linear_ToH.py| and
in \verb|dyadic_linear_ToH.py|.
