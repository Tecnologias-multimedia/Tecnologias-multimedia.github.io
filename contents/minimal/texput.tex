...
\href{https://pipewire.org/}{Pipewire}, in Windows: \href{https://en.wikipedia.org/wiki/Windows_legacy_audio_components#Multimedia_Extensions_(MME)}{MME}, \href}https://en.wikipedia.org/wiki/DirectSound}{DirectSound}, \href{https://en.wikipedia.org/wiki/Technical_features_new_to_Windows_Vista#Audio_stack_architecture}{WASAPI}, or \href{https://en.wikipedia.org/wiki/Windows_Driver_Model}{WDM-KS}, in macOS:  ).
  ...
  \footnote{In the case of Xubuntu, one of the recommended operating
    systems for running InterCom), this is the audio server that comes
    with \href{https://www.xfce.org/}{Xfce})}.
\item .
\end{enumerate}

\section{Real-time audio processing alternatives}

Using sounddevice (and in general, for any real-time audio-processing
application), we have two alternatives for implementing an application
such as InterCom:
...
\item \verb|send(packed_chunk)| sends the packet to our interlocutor
  (see
  \href{https://github.com/Tecnologias-multimedia/InterCom/blob/master/src/minimal.py#L92}{minimal}). When
  ...
  \href{https://github.com/Tecnologias-multimedia/InterCom/blob/master/src/minimal.py#L92}{our
    case}), this step is not blocking nor CPU-bounding (the CPU usage
  is very low), as long as the number of packets/second is small and
  the sizes of the payloads are also small, as it is expected in
  InterCom.
