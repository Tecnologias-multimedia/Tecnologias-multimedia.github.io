...
his/her speaker, which can be captured again (some time later) by his
...
Decrease the gain of the amplifier of our speaker to do
...
small.\footnote{Take in mind that convolution is a $O\log_2O$
...
ADALINE is able to learn hot to distinguish between patterns, even
...
\item In a ``$s(t)$ delay and substract solution'', if you also
  ...
  number (the Fourier coefficients are complex number).
...
\href{https://en.wikipedia.org/wiki/Correlation}{correlation} can help
in the task of fine-tunning $d$.
