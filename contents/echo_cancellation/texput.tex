...
\url{https://www.youtube.com/watch?v=hc2Zj55j1zU}} Using LMS, ADALINE
is able to learn pattern, even using a part of a single
neuron\footnote{If we do not consider the
...
\item (Optional) Use any other technique (for example, an artificial
  neural network\footnote{ADALINE is the simplest
  ...
  the echo. In this case, consider to process the signals by chunks,
  not by samples.
\end{enumerate}
