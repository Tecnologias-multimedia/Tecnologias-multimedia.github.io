...
mic(rophone), ${\mathbf n}$ is the signal emited by
...
it's $t$-the sample\footnote{Or frame if we work in stereo}. In this
solution, we send
\begin{equation}
...
our speaker to our mic. We define
\begin{equation}
...
minimize the mean square error of a target function. LMS is adaptive,
...
\item Finally, try to use LMS to find a estimation of the echo signal
  that takes into consideration the impulse response of the local-end
  audioset.
\end{enumerate}
