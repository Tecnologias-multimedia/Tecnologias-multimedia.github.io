...
\begin{enumerate}
\item To know if you can use
InterCom to communicate between your home networks (or between
simulated private networks).
\item To classify your NAT device.
\end{enumerate}

\subsection{Testing your NAT devices}

Run an instance of InterCom in two different private networks, and try
to communicate. Use the command line parameters to specify the
end-point of your interlocutor. Report your experiment. Try to use
...
NAT devices can be classified into:
\begin{enumerate}
\item Full cone NATs: when the end-point usigned by the NAT to
  InterCom remains the same for two (or more) interlocutors (each one
  in a different private network), and none packet filtering policy is
  used.
\item Restricted cone NATs: when in the previous situation (full cone) 
\item Port-restricted cone NATs:
\item Symmetric NATs:
\end{enumerate}

\subsection{Simulating a NATted environment}

InterCom should be able to communicate in all NAT combinations except when both are symmetric.

If you cannot communicate, and this is 
