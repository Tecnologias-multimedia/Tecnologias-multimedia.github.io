...
${\mathbf w}=\begin{bmatrix} {\mathbf w}_0 & {\mathbf
    w}_1\end{bmatrix}^{\text T}$ (where for this transform,
$\mathbf{w}\in\mathbb{Z}^2$) is the corresponding
...
per frame. Notice that ${\mathbf x}\in\mathbb{Z}^2$ is a vector
space\footnote{Adding two vectors in the plane produces a third one
  also in the plane; multiplying a vector by a real scalar produces a
  second vector also in the plane. These two ingrained facts make the
  integer/real plane be a vector
  space.~\cite{vetterli2014foundations}} if we consider also the
required operations.
...
(Mid/Side Transform). The MST is similar to the $2\times 2$ KLT
...
\href{https://en.wikipedia.org/wiki/Correlation_and_dependence}{statistical
  correlation} between the subbands is minimal, and the
...
$f'$ denotes the derivative of $f$, the RD curve). The main drawback
of this approach is that the finding of $f_i$ is computationally
intensive (we must analyze, quantize, compress, decompress,
dequantize, synthesize, and compute the distortion of the data for a
sufficiently high number of quantization steps, in order to determine
a good bit-rate $x$), and usually we cannot do that in real
...
other, and this is true because the MST is orthogonal.
...
to repeat themselves, especially locally. Another clear source of
temporal redundancy is that neighboring audio samples usually show
similar amplitude values.

% A decorrelating technique: DPCM
There are several techniques that can be used to remove the
temporal redundancy of a sequence of audio. A
...
\href{https://en.wikipedia.org/wiki/Transform_coding}{Transform
  Coding}, such as the one previously described.
...
\href{https://web.stanford.edu/class/ee398a/handouts/lectures/07-TransformCoding.pdf}{Transform
  Coding Gain}~\cite{sayood2017introduction}, accumulating most of the
...
blocks of samples. In this context, our analysis transform matrix
${\mathbf K}$ (see the Section~\ref{sec:analysis_transform})
represents the coefficients of a 2-channels analysis
...
(without splitting $\mathbf{x}$ into blocks as happens in ``pure''
Transform Coding). Let us also suppose (as happens in the MST) that
...
(analysis/synthesis) transform is named a (2-channels)
...
\href{https://github.com/Tecnologias-multimedia/Tecnologias-multimedia.github.io/blob/master/contents/transform_coding/PRFB.ipynb}{A
  ...
  this downsampling is possible because the
...
\href{https://en.wikipedia.org/wiki/Quadrature_mirror_filter}{Quadrature
  Mirror Filters (QMF) Bank}. Notice that the downsamplers ($\downarrow 2$) expand the spectrum of the input in a factor of two, but do not modify their shape, and the upsamplers ($\uparrow 2$) compress the spectrum of the input in a factor of two, generating a mirrored version of the half low-frequencies in the half high-frequencies.
...
\begin{figure}
  ...
  \caption{A dyadic 2-levels cascade of PRFBs (DWT).}
  \label{fig:cascade}
\end{figure}
...
\item Lifting is based on the concept of dyadic
  ...
  the downsampled (also refered as decimated) subbands computed by the
  corresponding 1-level PRFB, and we say, therefore, that
  Eq.~\eqref{eq:1dwt} computes the 1-level (critically sampled) DWT.
  ...
  $l=n$, where $N=2^n$, we have the spectrum partition (decomposition)
  \begin{equation*}
...
\item All DWT perform a number of lifting steps, each one with
  ...
  \item A \textbf{predict step}, which computes the ${\mathbf h}$
    subbands as a prediction error (that should be minimized if we
    want to obtain a good transform coding gain) between the even
    samples (usually, the values used to predict) and the odd samples
    (usually, the values predicted). For the MST filters, we have
    (see Eq.~\eqref{eq:ldwt}) that
    \begin{equation}
      {\mathbf h}^l_i = {\mathbf l}^{l-1}_{2i+1} - {\mathbf l}^{l-1}_{2i}.
    \end{equation}
  ...
  \begin{equation}
    ...
    \end{array}
  \end{equation}
  i.e., we can recover the signal samples from the coefficients.
...
response of the low-pass analysis filter (${\mathbf K}_0$ in MST) to
the
\href{https://en.wikipedia.org/?title=Unit_impulse&redirect=no}{unit
  ...
  of coefficients of the filter.} is known as \emph{scaling function}
and is usually denoted by $\tilde\phi$, the response of the analysis
...
represented by $\psi$, which are the dual scaling and wavelet
functions (see the notebook
\href{https://github.com/Tecnologias-multimedia/Tecnologias-multimedia.github.io/blob/master/contents/transform_coding/explore_wavelet_filters.ipynb}{Explore
  wavelet filters}).

For all the orthogonal DWTs, it holds that $\tilde\phi\bot\tilde\psi$,
the frequency response of $\tilde\phi$ is equal to the
mirror\footnote{In the $Z$-domain, it holds that
  ${\mathcal Z}\{\tilde\phi\}(z)={\mathcal Z}\{\phi\}(-z)$.} of
frequency response of $\phi$, and the frequency response of
$\tilde\psi$ is equal to the mirror of the frequency of of
$\psi$. Another important characteristic of orthogonal DWTs is that
...
which we use a 1-order predictor to remove redundancy. Let us extend
the idea of lifting to a prediction of order two. For that, we define
the predict step as
\begin{equation}
...
they only satisfy $\psi\bot\tilde\phi$ and
$\phi\bot\tilde\psi$, but the filters can be symmetric.
...
(see Section~\ref{sec:quantization_subbands_spatial} is applicable
here: we should select the different quantization step sizes depending
on the slope generated in the rate/distortion curve (see
...
\section{Wavelet Packets Transform (WPT)}

The WPT (see Fig.~\ref{fig:WPT}) is an extension of the DWT where the
PRFB are applied also to the high frequency subbands, generating a
linear partitioning of the spectrum. If $l$ is the number of levels,
the number of subband is $2^l$.

\begin{figure}
  \centering
  \myfig{graphics/WPT}{8cm}{800}
  \caption{A 2-levels WPT.}
  \label{fig:WPT}
\end{figure}

\section{\href{https://en.wikipedia.org/wiki/Lapped_transform}{Overlapped block transforms}}
%{{{
...
The MDCT is an overlapped transform where the basis functions (the
filters of the analysis matrix) are cosine function samplings of
different frequencies. These functions are defined by
\begin{equation}
  \mathbf{w}_k = \sum_{n=0}^{2N-1}\cos\left[\frac{\pi}{N} \left(n + \frac{1}{2} + \frac{N}{2}\right) \left(k + \frac{1}{2}\right) \right],
\end{equation}
where $N$ is the number of samples to transform.

The MDCT is orthogonal and therefore, the inverse transform only
differs from the forward one in a normalization coefficient of value
$1/N$.

\subsection{Overlapping}
The MDCT is overlapped using a technique called Time-Domain Aliasing
Cancellation (TDAC). If the MDCT block length (chunk) is $N$, the transform
actually operates on $2N$ input samples and produces $N$ output
coefficients. Example for 2048 samples:
...
The inverse transform input $N$ coefficients and output $2N$
samples. The reconstructed blocks are added overlapping $50\%$ ($N$
samples).

\subsection{Windowing functions}
To minimize the spectral leakage\footnote{Spectral leakage is a
phenomenon that occurs when a finite-length signal is analyzed using
infinite support functions (such as the used by the Fourier Transform,
or the Cosine Transform). Basically is caused because to generate a
finite-length signal (a block) is we must multiply the original
(supposely) infinite-length signal by a rectangular window whose
Fourier Transform is a Sinc function. Because this function is less
compact than other functions, the leakage can be minimized if we use
windows whose frequency response are more similar to the inpulse
function.}, the blocks are multiplied by a window function.

\section{Deliverables}

%\begin{enumerate}

Find the following RD curves:

\begin{enumerate}

\item \textbf{Determine the RD curves for the MST}: As we did in the
  previous milestone, generate the RD curves for a set of simulated
  transmission contexts. Use the modules
...
\item \textbf{Determine the RD curves for a
  DWT~\cite{lee2019pywavelets}}: Rebuild the RD curves considering
  also the removal of the temporal decorrelation. Use
  \verb|temporal_overlapped_DWT_coding.py| and the
  \verb|--minimal_quantization_step| parameter to generate the
  different points of the RD curves. Try to minimize the RD curve
  playing with the number of levels and the wavelet name.


\item \textbf{Determine the RD curves for the corresponding
  WPT~\cite{lee2019pywavelets}}: Compare with
  \verb|temporal_overlapped_WPT_coding.py|. Use always the same graph.

\item \textbf{Determine the RD curves for the MDCT}: Create a new
  module \verb|temporal_MDCT_coding.py| written from a notebook called
  \verb|temporal_MDCT_coding.ipynb|. Include in this module, as an
  option, at least one window function different to the rectangular
  window. Compare the MDCT in the R/D domain with the rest of
  transforms (MST, DWT, and WPT) over the same graph, considering
