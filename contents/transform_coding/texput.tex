...
In addition to verifying that ${\mathbf K}^{-1}={\mathbf K}^{\text T}$,
...
\href{https://en.wikipedia.org/wiki/Correlation_and_dependence}{statistical
  correlation} between the subbands is 0, and therefore the contributions
...
satisfied by many famous transforms (such as the
...
Eq.~\eqref{eq:inverse_transform_matrix_form}). In contrast, we would
...
As a final remark, we could also consider that any alternative other than $\Delta_0=\Delta_1$ will affect the spatial perception of the audio. The notebook \href{https://github.com/Tecnologias-multimedia/Tecnologias-multimedia.github.io/blob/master/study_guide/transform_coding/stereo_transforms_RD.ipynb}{A
...
spatial redundancy.}. As can be seen in the notebook
...
As has been explained before, Transform Coding is based on the idea
...
\item
  ...
  faster than that based on vector-matrix arithmetic. And most
...
(see Fig.~\ref{fig:3_chunks}). This is a consequence of the
prediction step used by the DWT in the limits of the chunks, generating
...
such an impact. Experiment also with the wavelet name.
